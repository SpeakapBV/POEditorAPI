...
func processJSON(data: Data, outputFolderList: [URL]) throws
{
    ...
    for translation in translations ?? []
    {
        ...
        if let single = translation["definition"] as? String, !single.isEmpty
        {
            translated = TranslatedTerm.hasDefinition(single)
        }
        else if let plurals = translation["definition"] as? [String: String], hasTranslationsFor(plurals)
        {
...
func hasTranslationsFor(_ plurals: [String: String]) -> Bool {
    let pluralTypes = plurals.keys

    for pluralType in pluralTypes {
        if let plural = plurals[pluralType], plural.isEmpty {
            return false
        }
    }
    return true
